...
- `GEMINI_API_KEY=... docker compose -f compose.claude-with-litellm.yml run --rm claude-code -p "explain this project"` runs Claude Code through LiteLLM with Gemini-backed model aliases.
- `OPENROUTER_API_KEY=... ANTHROPIC_DEFAULT_SONNET_MODEL=openrouter-sonnet LITELLM_OPENROUTER_SONNET_MODEL=openrouter/openai/gpt-4.1 docker compose -f compose.claude-with-litellm.yml run --rm claude-code -p "explain this project"` routes the Sonnet slot through an OpenRouter override.
...
- Keep filenames lowercase (`compose.claude.yml`, `compose.yml`, `compose.claude-with-codex.yml`, `compose.claude-with-litellm.yml`, `ci.yml`) and use descriptive, tool-oriented names.
